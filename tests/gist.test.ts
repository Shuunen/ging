...
check('gist file contains FILE_NAME', fileA[FILE_NAME].content, JSON.stringify(gistStateA, undefined, 2))
...
const fetch = {
  fail: async (_url: RequestInfo | URL, _options?: RequestInit): Promise<Response> => {
    return { json: async () => ({ message: fetch.failMessage }) } as never
  },
  failMessage: 'a fake server error occurred',
  failReadEmpty: async (_url: RequestInfo | URL, _options?: RequestInit): Promise<Response> => {
    return { json: async () => (fetch.failReadEmptyContent) } as never
  },
  failReadEmptyContent: { files: {} },
  success: async (_url: RequestInfo | URL, _options?: RequestInit): Promise<Response> => {
    return { json: async () => (fetch.successContent) } as never
  },
  successContent: { id },
  successList: async (_url: RequestInfo | URL, _options?: RequestInit): Promise<Response> => {
    return { json: async () => (fetch.successListContent) } as never
  },
  successListContent: [{ id, files: { [FILE_NAME]: { content: JSON.stringify(gistStateA, undefined, 2) } } }],
  successListEmpty: async (_url: RequestInfo | URL, _options?: RequestInit): Promise<Response> => {
    return { json: async () => (fetch.successListEmptyContent) } as never
  },
  successListEmptyContent: [],
  successRead: async (_url: RequestInfo | URL, _options?: RequestInit): Promise<Response> => {
    return { json: async () => (fetch.successReadContent) } as never
  },
  successReadContent: { files: { [FILE_NAME]: { content: JSON.stringify(gistStateA, undefined, 2) } } },
}
...
check('gist read fails with empty gist', read(id, token, fetch.failReadEmpty), { success: false, message: 'gist read failed to find ' + FILE_NAME })
