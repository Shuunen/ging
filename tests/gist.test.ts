...
check('gist getId fails', getId('', { isGistState: false, projects: [] }, '', fetch.fail), { success: false, message: fetch.failMessage })
check('gist getId success by finding an existing one', getId('', { isGistState: false, projects: [] }, '', fetch.successList), { success: true, message: 'gist id found', data: id })
...
check('gist getId success by creating a new one', getId('', { isGistState: false, projects: [] }, '', fetch.successListEmpty), { success: false, message: 'POST request on https://api.github.com/gists succeed' })
check('gist getId with an existing one', getId(id, { isGistState: false, projects: [] }, '', fetch.successList), { success: true, message: 'gist id already set', data: id })
...
check('gist persist fails with empty token', persist('reason B', '', { isGistState: false, projects: [] }, '', fetch.success), { success: false, message: 'Cannot save your work without a Gist token' })
// same tricky check as above
check('gist persist update with a token', persist('reason C', '', { isGistState: false, projects: [] }, token, fetch.successList), { success: false, message: 'PATCH request on https://api.github.com/gists/aUnitTest_gist_id succeed' })
